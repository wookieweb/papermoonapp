...
{
    // Load 50 recent photos only...
    NSString *request = [NSString stringWithFormat:@"http://api.flickr.com/services/rest/?method=flickr.photos.search&per_page=50&sort= interestingness-desc&license=1,2,4,7&has_geo=1&extras=original_format,tags,description,geo,date_upload,owner_name,place_url"];
...
+ (NSArray *)paperMoonPhotos
{
  //  NSString *request = [NSString stringWithFormat:@"http://api.flickr.com/services/rest/?method=flickr.photos.search&per_page=25&sort= interestingness-desc&extras=original_format,tags,description,geo,date_upload,owner_name,place_url&tags=papermonapp"];
    
    NSString *request = [NSString stringWithFormat:@"http://api.flickr.com/services/rest/?method=flickr.photos.search&per_page=50&sort= interestingness-desc&extras=original_format,tags,description,geo,date_upload,owner_name,place_url&tags=papermoonapp"];
  //  NSLog(@"Return from FlickrFetcher");
 //   NSLog(@"content %@", [[self executeFlickrFetch:request] valueForKeyPath:@"photos.photo"]);
    return [[self executeFlickrFetch:request] valueForKeyPath:@"photos.photo"];
}
...
+ (NSArray *)topPlaces
{
    NSString *request = [NSString stringWithFormat:@"http://api.flickr.com/services/rest/?method=flickr.places.placesForTags&tags=hongkong&place_type_id=7"];
    return [[self executeFlickrFetch:request] valueForKeyPath:@"places.place"];
}
